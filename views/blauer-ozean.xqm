...
function _:sanofi-blauer-ozean-content-view($KK as element(kk), $Schema as element(schema), $Context as map(*))
as element(xhtml:div)
{
let $id := $KK/@id/string()
let $context := map{"context":"sanofi/blauer-ozean"}
let $schema := plugin:provider-lookup("sanofi/blauer-ozean","schema")!.()
let $items := plugin:provider-lookup("sanofi/blauer-ozean","datastore/dataobject/all")!.($schema,$context)[kk=$KK/@id]
let $item := $items[name=$Context("name")]
...
let $soll := string-join(for $x in $soll-names return $item/element()[name()=$x]/string(),",")
let $labels := '"'||string-join($schema/element[ends-with(@name,"-ist")]/label/substring-before(string()," IST"),'","')||'"'
let $edit-button := try {plugin:provider-lookup("sanofi/blauer-ozean","schema/render/button/modal/edit")!.($item,$schema,$context)} catch * {}
return
<div xmlns="http://www.w3.org/1999/xhtml" class="content-with-sidebar row">
  <div class="row">
      <div class="col-lg-12 col-md-12">
          <div class="ibox float-e-margins">
              <div class="ibox-title">
                  <h5>{$edit-button} {$name}</h5>
                  <select class="chosen pull-right" onchange="restxq('/influx/sanofi/blauer-ozean?id='+$(this).val())">
                    <option>{if (not($id)) then attribute selected {} else ()}Bitte auswählen</option>
                    {$items ! <option value="{./@id/string()}">{if ($id=./@id) then attribute selected {} else ()}{./*:name/string()}</option>}
                  </select>
              </div>
